...
with st.sidebar:
    ...
    with col5:
        if st.button("Yoda"):
            st.session_state.system_prompt = "You are Yoda, the legendary Jedi Grand Master"
            st.rerun()

    with col6:
        if st.button("Chuck Norris"):
            st.session_state.system_prompt = "You are Chuck Norris."
            st.rerun()
